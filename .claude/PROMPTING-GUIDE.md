# Claude Code Prompting Guide for LániCAD
...
```markdown
## Current Phase
Phase 1: Stabilize & Ship — See ROADMAP.md for details.
...
- Do not create new pages — all routes are defined, focus on improving existing ones
- Do not add English text to the UI — everything must be Icelandic
...
- server/index.js — Express API, currently migrating to Turso
- src/lib/db.ts — Frontend data layer
- src/pages/calculators/*.tsx — Calculator pages

### Stable (don't touch unless asked)
- src/data/*.ts — Product data (verified against Excel sources)
- src/lib/calculations/rental.ts — Core formulas (137 tests cover these)
...
- `@src/pages/calculators/FenceCalculator.tsx` — points Claude to a specific file
- `@src/lib/` — points to a directory

### Use /commands
- `/commit` — Let Claude create a commit with a good message
...
1. **Don't say "improve"** — Be specific about what to improve and how
2. **Don't say "fix everything"** — Point to one specific bug at a time
3. **Don't skip the read step** — Always have Claude read files before editing
4. **Don't forget build verification** — Always end with `npm run build`
5. **Don't let Claude add dependencies** — Ask it to check if existing deps can solve the problem first
6. **Don't accept large refactors** — If Claude wants to restructure a working file, push back
7. **Don't forget Icelandic** — Remind Claude that all UI text must be in Icelandic
